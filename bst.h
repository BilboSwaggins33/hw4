...
void BinarySearchTree<Key, Value>::remove(const Key &key) {
  // TODO
  Node<Key, Value> *item = internalFind(key);
  ...
  int numChildren =
      (item->getLeft() != nullptr) + (item->getRight() != nullptr);

  if (numChildren == 2) {

    Node<Key, Value> *pred = predecessor(item);
    nodeSwap(item, pred);
    Node<Key, Value> *parent = item->getParent();

    bool isLeft = parent->getLeft() == item;

    if (item->getRight() == nullptr && item->getLeft() == nullptr) {
      ...
      if (item->getRight() != nullptr) {
        isLeft ? parent->setLeft(item->getRight())
               : parent->setRight(item->getRight());
        item->getRight()->setParent(parent);
      } else {
        isLeft ? parent->setLeft(item->getLeft())
    ...
    Node<Key, Value> *parent = item->getParent();
    Node<Key, Value> *child;
    bool childLeft = item->getLeft() != nullptr;
    if (childLeft) {
      child = item->getLeft();
    ...
    if (parent != nullptr) {
      bool itemLeft = parent->getLeft() == item;
...
template <typename Key, typename Value> int getHeight(Node<Key, Value> *root) {
  if (root == nullptr) {
    return 0;
  }
...
template <typename Key, typename Value>
bool balancedHelper(Node<Key, Value> *root) {
  if (root == nullptr) {
    return true;
  }
